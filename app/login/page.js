  ...
  return (
    <main className="min-h-screen bg-gray-950 flex items-center justify-center">
      <div className="bg-gray-900 p-8 rounded-xl w-full max-w-md">
        ...
        <button onClick={handleLogin} className="w-full bg-red-500 text-white py-3 rounded-lg font-medium hover:bg-red-600">Se connecter</button>
        {message && <p className="text-red-400 mt-4 text-center">{message}</p>}
        <p className="text-gray-400 text-center mt-4">Pas de compte ? <Link href="/register" className="text-red-400">S'inscrire</Link></p>
      </div>
    </main>
  )
}